...
describe('audit catalog',()=>{
 it('contains the complete expanded check catalog',()=>expect(CHECK_CATALOG).toHaveLength(360));
 it('has unique stable IDs',()=>expect(new Set(CHECK_CATALOG.map(x=>x.id)).size).toBe(360));
