...
export async function fetchPage(url:string):Promise<PageData> {
  ...
  try {
    ...
    return {url:pageUrl,status:res.status,contentType:res.headers.get('content-type')||'',html,title:cleanText($('title').first().text()),description:cleanText($('meta[name="description"]').attr('content')||''),h1:$('h1').map((_,el)=>cleanText($(el).text())).get(),headings,links,images,scripts,stylesheets,canonical,lang:$('html').attr('lang')||null,wordCount:text?text.split(/\s+/).length:0,loadMs:Date.now()-start,headers:headersToObject(res.headers),cookies,sourceSize:Buffer.byteLength(html)};
...
export async function crawlSite(startUrl:string,maxPages=DEFAULT_MAX):Promise<{pages:PageData[],robots:string, sitemapUrls:string[], discovered:string[]}> {
  ...
  while(queue.length && pages.length<maxPages){
    ...
    pages.push(p);
    for(const l of p.links){ if(!l.external && l.href.startsWith(start.origin) && !seen.has(l.href) && queue.length<maxPages*3) queue.push(l.href); }
  }
